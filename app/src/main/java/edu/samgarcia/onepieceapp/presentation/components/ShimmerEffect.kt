        ...
        Column(
            ...
            Surface(
                modifier = Modifier
                    .alpha(alpha)
                    .fillMaxWidth(0.5f)
                    .height(SHIMMER_NAME_HEIGHT),
                color = if (isSystemInDarkTheme()) ShimmerDarkGray else ShimmerMediumGray,
                shape = RoundedCornerShape(size = XXS_PADDING)
            ) {}

            Spacer(modifier = Modifier.padding(all = XS_PADDING))
            ...
            repeat(3) {
                Surface(
                    modifier = Modifier
                        .alpha(alpha)
                        .fillMaxWidth()
                        .height(SHIMMER_ABOUT_HEIGHT),
                    color = if (isSystemInDarkTheme())
                        ShimmerDarkGray else ShimmerMediumGray,
                    shape = RoundedCornerShape(size = XXS_PADDING)
                ) {}

                Spacer(modifier = Modifier.padding(all = XXS_PADDING))
            }
